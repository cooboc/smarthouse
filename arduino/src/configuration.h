...
class Configuration {
  public:
    ...
    void updateConfiguration(const char *ssid,
                             const char *password,
    ...
    void updateConnectivity(std::uint8_t c);
    void commitUpdate();
    void resetSettings();

    bool isWifiCredentialExists() const { return (std::strlen(persistent_.wifiSsid) > 0); }

    inline const char *getWifiSsid(void) const { return persistent_.wifiSsid; }
    inline const char *getWifiPassword(void) const { return persistent_.wifiPassword; }
    inline const char *getServerAddr(void) const { return persistent_.serverAddr; }
